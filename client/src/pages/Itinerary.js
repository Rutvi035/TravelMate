...
function Itinerary() {
  ...
  // Fetch all itineraries for logged-in user (or all users if admin)
  const fetchItineraries = async () => {
    try {
      const url = isAdmin
        ? "http://localhost:5000/api/admin/itineraries"
  ...
  return (
    <div className="container">
      <div className="page-header">
        <h2>🗓️ {isAdmin ? "All User Itineraries" : "My Itineraries"}</h2>
        <p>
          {isAdmin
            ? "View itineraries created by all users"
            : "Plan your trip schedule day by day"}
        </p>
      </div>

      {/* Create Itinerary Form — hidden for admin */}
      {!isAdmin && (
      ...
      {itineraries.length === 0 ? (
        ...
        itineraries.map((itin) => (
          <div className="card fade-in" key={itin._id}>
            ...
            {/* Add Item Form — hidden for admin */}
            {!isAdmin &&
              (selectedId === itin._id ? (
                <div
                  style={{
